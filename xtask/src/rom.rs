...
pub fn do_pack(
    ...
    let header = build_header(platform, rom_version, flags, required_features,
        payload.len() as u32, checksum);
...
pub fn do_verify(input: &Path) -> Result<(), String> {
    ...
    println!("magic:        {} {}", hex::encode(&hdr[OFF_MAGIC..OFF_MAGIC+6]), tick(magic_ok));

    let spec = u16::from_le_bytes([hdr[OFF_SPEC_VERSION], hdr[OFF_SPEC_VERSION+1]]);
    println!("spec_version: {} {}", spec, tick(spec == SPEC_VERSION));
    ...
    let all_ok = magic_ok && spec == SPEC_VERSION && term_ok
        && hdr_size == HEADER_V1_SIZE && checksum_ok;
...
mod tests {
    ...
    fn pack_and_verify_round_trip() {
        ...
        do_pack("esp32", "0.1.0", None, false, input.path(), output.path()).unwrap();
    ...
    fn output_size_is_header_plus_payload() {
        ...
        let output = NamedTempFile::new().unwrap();
        do_pack("esp32", "1.2.3", None, false, input.path(), output.path()).unwrap();
    ...
    fn features_round_trip() {
        ...
        let output = NamedTempFile::new().unwrap();
        do_pack("esp32", "0.1.0", Some("psram,wifi"), false, input.path(), output.path()).unwrap();
    ...
    fn pack_rejects_empty_input() {
        let input  = write_tmp(&[]);
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp32", "0.1.0", None, false, input.path(), output.path()).is_err());
    }
    ...
    fn pack_rejects_bad_platform() {
        ...
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp999", "0.1.0", None, false, input.path(), output.path()).is_err());
    }
    ...
    fn version_components_must_be_0_to_255() {
        let input  = write_tmp(&[1, 2, 3]);
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp32", "0.256.0", None, false, input.path(), output.path()).is_err());
    }
}
