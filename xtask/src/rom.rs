...
pub fn do_pack(
    ...
    let built_against = match built_against_str {
        Some(s) => { let v = parse_version(s)?; version_pack(v[0], v[1], v[2]) }
    ...
    let payload_type = match payload_type_str {
        ...
        Some(other) => return Err(format!("unknown payload type '{other}': use native | wasm32 | luac54")),
    };
    ...
    let header = build_header(
        platform, rom_version, built_against, flags, required_features,
        payload.len() as u32, payload_type, rom_id, [0, 0, 0], checksum,
    );
    ...
    println!("rom-id:        {}", if rom_id.is_empty() { "(none)" } else { rom_id });
...
pub fn do_verify(input: &Path) -> Result<(), String> {
    ...
    println!("magic:          {} {}", core::str::from_utf8(&hdr[OFF_MAGIC..OFF_MAGIC + 4]).unwrap_or("????"), tick(magic_ok));
    ...
    let compat_names: Vec<&str> = compat.iter()
        .filter(|&&b| b != 0)
    ...
    println!("payload_type:   0x{:02X} ({}) {}", hdr[OFF_PAYLOAD_TYPE],
        ptype.map(|t| t.name()).unwrap_or("unknown"), tick(ptype_ok));

    let rom_id_bytes = &hdr[OFF_ROM_ID..OFF_ROM_ID + ROM_ID_LEN];
    let rom_id_end = rom_id_bytes.iter().position(|&b| b == 0).unwrap_or(ROM_ID_LEN);
    ...
    println!("header_end:     {} {}", core::str::from_utf8(&hdr[OFF_HEADER_END..OFF_HEADER_END + 4]).unwrap_or("????"), tick(end_ok));
    ...
    println!("computed crc32:  0x{:08X} {}", actual_crc, tick(checksum_ok));

    let all_ok = magic_ok && api_ok && end_ok && hdr_size == HEADER_V1_SIZE && checksum_ok && ptype_ok;
...
pub fn parse_platform(s: &str) -> Result<u8, String> {
    match s.to_lowercase().as_str() {
        ...
        "any"                => Ok(PLATFORM_ANY),
        other => Err(format!("unknown platform '{other}': use esp32 | esp32-s3 | rp2040 | any")),
    }
}
...
mod tests {
    ...
    fn pack_and_verify_round_trip() {
        ...
        do_pack("esp32", "0.2.0", None, None, None, None, false, input.path(), output.path()).unwrap();
    ...
    fn output_size_is_header_plus_payload() {
        ...
        let output = NamedTempFile::new().unwrap();
        do_pack("esp32", "0.2.0", None, None, None, None, false, input.path(), output.path()).unwrap();
        assert_eq!(std::fs::read(output.path()).unwrap().len(), HEADER_V1_SIZE + 1000);
    }
    ...
    fn features_round_trip() {
        ...
        let output = NamedTempFile::new().unwrap();
        do_pack("esp32", "0.2.0", None, Some("psram,wifi"), None, None, false, input.path(), output.path()).unwrap();
    ...
    fn pack_rejects_empty_input() {
        let input  = write_tmp(&[]);
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp32", "0.2.0", None, None, None, None, false, input.path(), output.path()).is_err());
    }
    ...
    fn pack_rejects_bad_platform() {
        ...
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp999", "0.2.0", None, None, None, None, false, input.path(), output.path()).is_err());
    }
    ...
    fn version_components_must_be_0_to_255() {
        let input  = write_tmp(&[1, 2, 3]);
        let output = NamedTempFile::new().unwrap();
        assert!(do_pack("esp32", "0.256.0", None, None, None, None, false, input.path(), output.path()).is_err());
    }
    ...
    fn pack_wasm_payload_type_stored() {
        let input  = write_tmp(&vec![0u8; 64]);
        let output = NamedTempFile::new().unwrap();
        do_pack("esp32", "0.2.0", None, None, Some("wasm32"), Some("com.test.app"), false, input.path(), output.path()).unwrap();
        let data = std::fs::read(output.path()).unwrap();
    ...
    fn pack_rejects_too_long_rom_id() {
        ...
        assert!(do_pack("esp32", "0.2.0", None, None, None, Some(long_id), false, input.path(), output.path()).is_err());
    }
}
