...
The v1 header is exactly **64 bytes**. The payload begins immediately after at byte `header_size` (64 for v1). The `header_size` field and `header_end` terminator make the format forward-compatible: future spec versions can extend the header beyond 64 bytes, and a loader always knows exactly where the payload starts.

| Field | Offset | Type | Size | Value / Notes |
|---|---|---|---|---|
| `magic` | `0x00` | `u8[6]` | 6 | ASCII `BROM\x00\x01` |
| `spec_version` | `0x06` | `u16 LE` | 2 | Flashpoint spec version. Currently `1`. |
| `platform` | `0x08` | `u8` | 1 | `0x01`=ESP32  `0x02`=ESP32-S3  `0x03`=RP2040  `0xFF`=multi (future) |
| `rom_version` | `0x09` | `u8[3]` | 3 | Semantic version `[major, minor, patch]` |
| `flags` | `0x0C` | `u16 LE` | 2 | Bit 0: compressed payload. Bits 1–15: reserved, must be 0. |
| `required_features` | `0x0E` | `u64 LE` | 8 | Bitmask of hardware features this boot-rom requires (see §4.4) |
| `payload_len` | `0x16` | `u32 LE` | 4 | Length of payload in bytes |
| `checksum` | `0x1A` | `u8[32]` | 32 | SHA-256 of payload bytes |
| `header_size` | `0x3A` | `u16 LE` | 2 | Total header size in bytes. `64` for v1. Payload starts at this offset. |
| `reserved` | `0x3C` | `u8[3]` | 3 | Must be zero. |
| `header_end` | `0x3F` | `u8` | 1 | Must be `0xFE`. Always the last byte of the header block. |
| **Total (v1)** | | | **64 bytes** | Payload begins at byte `header_size` |

**Forward compatibility:** a loader reads `header_size` at offset `0x3A`, reads that many bytes, then verifies `header_end == 0xFE` at `header_size - 1`. A v1 loader that sees `header_size > 64` must reject with "unsupported header version" — it cannot safely interpret fields it doesn't know about.

Stage 1 validates all fields before executing anything. Any failure causes an immediate fallback. **The device cannot be bricked by a malformed `flashpoint.rom`.**
...
Multi-platform `boot-rom`s are exclusively `flashpoint.rom` SD files. They are too large to embed in most devices' internal flash.

### 4.4 Feature Flags

The `required_features` field in the boot-rom header is a `u64` bitmask. The `flash-rom` publishes a matching `provided_features` bitmask to a known memory location before jumping to Stage 2. Stage 1 enforces: `(provided & required) == required`. If the check fails, the boot-rom is rejected and Stage 1 falls back — no bad code ever runs.

| Bit | Constant | Meaning |
|-----|----------|---------|
| 0 | `FEAT_PSRAM` | External PSRAM / SPIRAM available |
| 1 | `FEAT_WIFI` | WiFi hardware present |
| 2 | `FEAT_BLE` | Bluetooth LE present |
| 3 | `FEAT_DISPLAY_TFT` | TFT / LCD display present |
| 4 | `FEAT_DISPLAY_EINK` | eInk display present |
| 5 | `FEAT_INPUT_TOUCH` | Touch input present |
| 6 | `FEAT_INPUT_BUTTONS` | Physical directional buttons present |
| 7 | `FEAT_BATTERY` | Battery / ADC monitoring present |
| 8 | `FEAT_USB_OTG` | USB OTG present |
| 9–63 | — | Reserved, must be 0 in `required_features` |

**Flash-rom capability declaration** — emitted as a compile-time constant by `flash-rom/build.rs` and published into a fixed memory location at the start of Stage 1:

```rust
// Declared in flash-rom/src/capabilities.rs, generated by flash-rom/build.rs
pub const DEVICE_FEATURES: u64 =
    FEAT_DISPLAY_TFT |   // ILI9341 on CYD
    FEAT_INPUT_TOUCH |   // XPT2046 resistive touch on CYD
    0;                   // No PSRAM, no WiFi, no battery on base CYD
```

A `boot-rom` that requires `FEAT_PSRAM` will be rejected on the base CYD but accepted on the Xteink X4. The error is surfaced before any boot-rom code executes.
...
- **Header is 64 bytes (v1 minimum).** `header_size` field at `0x3A` gives the actual size; `header_end = 0xFE` at `header_size - 1` terminates it. Payload starts at byte `header_size`. Future versions may extend the header; v1 loaders must reject `header_size > 64`.
- **Feature flags:** `flash-rom` publishes `provided_features` bitmask. `boot-rom` header declares `required_features`. Loader enforces `(provided & required) == required` before any boot-rom code runs.
- **SD boot file is `flashpoint.rom`** (not `sdboot.rom`).
...
- **Build orchestration:** xtask. `cargo xtask build-flash` / `build-rom` / `flash` handles multi-target compilation. `build.rs` files handle constants only.
